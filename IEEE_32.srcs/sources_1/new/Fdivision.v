...
mult754 M1(.A({{1'b0,8'd126,B[22:0]}}),.B(32'h3ff0f0f1),.clk(clk),.RES(temp1)); //verified
assign debug = {1'b1,temp1[30:0]};
ADD_SUB A1(.A(32'h4034b4b5),.OP(1'b0),.B({1'b1,temp1[30:0]}),.out(x0));

mult754 M2(.A({{1'b0,8'd126,B[22:0]}}),.B(x0),.clk(clk),.RES(temp2));
ADD_SUB A2(.A(32'h40000000),.OP(1'b0),.B({!temp2[31],temp2[30:0]}),.out(temp3));
...
ADD_SUB A4(.A(32'h40000000),.OP(1'b0),.B({!temp6[31],temp6[30:0]}),.out(temp7));
mult754 M7(.A(x2),.B(temp7),.clk(clk),.RES(x3));

assign Exponent = x3[30:23]+8'd126-B[30:23];
assign reciprocal = {B[31],Exponent,x3[22:0]};

mult754 M8(.A(A),.B(reciprocal),.clk(clk),.RES(result));
endmodule
